...
export default function PressDetailPage() {
  ...
  const post = posts.find((p) => p.id === id && p.category === '언론 보도자료');

  if (!post) {
    return (
      <div className="max-w-7xl mx-auto px-4 py-12">
        <p>게시글을 찾을 수 없습니다.</p>
        <Link to="/board/press" className="text-blue-600">
          목록으로
        </Link>
      </div>
    );
    ...
    <div className="max-w-4xl mx-auto px-4 py-12">
      <div className="bg-white rounded-lg shadow-lg p-8">
        ...
        <div className="border-t pt-4">
          <Link to="/board/press" className="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700">
            목록으로
          </Link>
        </div>
      </div>
